...
int main(void){
  ...
  fprintf(stderr,"A sem16\n"); int64_t z[64]; sem16(3,5,z); fprintf(stderr,"A done\n");
  ...
  fprintf(stderr,"B k16\n"); k16(1000,a16,b16); fprintf(stderr,"B done\n"); t0=now(); k16(it,a16,b16); t1=now(); n=(double)it*8;
  printf("SMOPA i16->i64: %.3f ns  %.2f G instr/s -> %.1f G int16x16 MAC/s\n",(t1-t0)/n*1e9,n/(t1-t0)/1e9,n*128/(t1-t0)/1e9);
  fprintf(stderr,"C k8\n"); k8(1000,a8,b8); fprintf(stderr,"C done\n"); t0=now(); k8(it,a8,b8); t1=now(); n=(double)it*4;
  printf("SMOPA i8->i32 : %.3f ns  %.2f G instr/s -> %.1f G int8 MAC/s (%.0f GOPS)\n",(t1-t0)/n*1e9,n/(t1-t0)/1e9,n*1024/(t1-t0)/1e9,n*2048/(t1-t0)/1e9);
  fprintf(stderr,"D kread\n"); int64_t buf[8]; kread(1000,buf); fprintf(stderr,"D done\n"); t0=now(); kread(1000000,buf); t1=now(); n=1000000.0*8;
  printf("ZA readout mova+st1d (64B/row): %.3f ns/row -> %.1f GB/s\n",(t1-t0)/n*1e9,n*64/(t1-t0)/1e9);
  fprintf(stderr,"E krt\n"); krt(1000); fprintf(stderr,"E done\n"); t0=now(); krt(500000); t1=now();
  printf("smstart+smstop round trip: %.1f ns\n",(t1-t0)/500000*1e9);
  fprintf(stderr,"F kmul\n"); uint64_t s; kmul(1000,&s); fprintf(stderr,"F done\n"); t0=now(); kmul(it,&s); t1=now(); n=(double)it*4;
