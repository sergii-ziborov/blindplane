...
/* A: reproduce the claim's per-op kernel (1 tile, targeted ST1D readout) */
...
/* B: BATCHED - 8 independent field muls into za0.d..za7.d, ONE bulk STR ZA drain.
   This is the construction the negative claim did not test. */
...
/* C: cost of `zero {za}` alone */
static void zero_only(int64_t iters){
  __asm__ volatile("smstart\n\t"
   "1:\n\t zero {za}\n\t subs %0,%0,#1\n\t bne 1b\n\t smstop\n\t"
   :"+r"(iters)::CL);}

/* D: SMOPA issue rate alone, no readout, 8 independent tiles */
...
/* E: smstart/smstop mode-switch cost */
static void modeswitch(int64_t iters){
  __asm__ volatile("1:\n\t smstart\n\t smstop\n\t subs %0,%0,#1\n\t bne 1b\n\t"
   :"+r"(iters)::CL);}

/* F: real scalar 5x51 field multiply (the actual baseline being replaced) */
...
int main(void){
  ...
  printf("A  1-tile kernel (claim's):      %7.2f ns/field-mul\n",(double)(t1-t0)/n);

  batch8(1000,a,b,buf); t0=nowi(); batch8(it/4,a,b,buf); t1=nowi(); n=(double)(it/4);
  printf("B  batch-8 + bulk STR ZA drain:  %7.2f ns/batch = %6.2f ns/field-mul\n",
     (double)(t1-t0)/n,(double)(t1-t0)/n/8.0);

  zero_only(1000); t0=nowi(); zero_only(it); t1=nowi();
  printf("C  zero {za} alone:              %7.2f ns\n",(double)(t1-t0)/(double)it);

  smopa_only(1000,a,b); t0=nowi(); smopa_only(it,a,b); t1=nowi(); n=(double)it*8;
  printf("D  SMOPA issue rate (no drain):  %7.3f ns/SMOPA -> %.1f G int16 MAC/s\n",
     (double)(t1-t0)/n, n*256/(double)(t1-t0));

  modeswitch(1000); t0=nowi(); modeswitch(it); t1=nowi();
  printf("E  smstart+smstop pair:          %7.2f ns\n",(double)(t1-t0)/(double)it);

  /* scalar baseline: serial dependent chain (ladder-like) */
  fe x={{1,2,3,4,5}},y={{6,7,8,9,10}};
  fe_mul(&x,&y);
  t0=nowi(); for(int64_t i=0;i<it*10;i++){ x=fe_mul(&x,&y); } t1=nowi();
  printf("F  scalar 5x51 fe_mul (serial):  %7.2f ns/field-mul  [sink %llu]\n",
     (double)(t1-t0)/(double)(it*10),(unsigned long long)x.l[0]);
  return 0;
}
